    ...
    return (
      <Widget>
        ...
        <Grid container spacing={3}>
          <Grid item xs={6} md={3} style={{ position: 'fixed', height: '100vh', overflowY: 'auto' }}> {/* xs 값을 변경하여 deviceID 카드의 크기를 줄였습니다. */}
            {allDevices.map(deviceID => (
              <Card 
                ...
                style={{ 
                  ...
                  margin: '10px 0',
                  cursor: 'pointer',
                  height: '150px'  // 카드의 높이를 제한하여 크기를 줄였습니다.
                }}
              >
                <CardContent>
          ...
          <Grid item xs={6} md={9}> {/* md 값을 변경하여 deviceID 카드와 device.device_id 카드의 공간 비율을 조절했습니다. */}
            {selectedDevice ? (
              <div>
                {isLoading ? (
                  <div>Loading...</div>
