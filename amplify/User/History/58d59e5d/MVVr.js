...
const DeviceViewer = () => {
  ...
  const callAPI = async () => {
    ...
    try {
      ...
      // Check if result is an array (since the response is directly an array of devices)
      if (Array.isArray(result)) {
          setDevices(result);
      } else {
          console.log("API Response:", result);  // For debugging
          setDevices([]);  // Empty array as default
      }
  ...
  return (
    <div>
      ...
      <div>
        ...
        />
        <Button variant="contained" color="primary" onClick={callAPI} style={{ margin: '20px 0' }}>
          Call API
        </Button>
      </div>
